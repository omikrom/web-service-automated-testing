...
{		
	...
	try
	{
		...
		let values = [];
		await client.connect();
		if(user.Username != null) { values.push(user.Username); queryArgs.push('"Username"=%L'); }
		if(user.Password != null) { values.push(user.Password); queryArgs.push('"Password"=%L'); }
		if(user.Email != null) { values.push(user.Email); queryArgs.push('"Email"=%L'); }
		if(user.Name != null) { values.push(user.Name); queryArgs.push('"Name"=%L'); }
